// Transcrypt'ed from Python, 2022-03-14 09:39:56
...
export var Index =  __class__ ('Index', [gatehandler.Handler], {
	...
	get initialize () {return __get__ (this, decorators.check_authorization ((function __lambda__ () {
		...
		if (arg0 == 'calendario-base') {
			...
			if (arg2 == 'editar' && window.PhanterPWA.auth_user_has_role (['administrator', 'root', 'Administrador Master SME', 'Administrador Master Escola', 'Coordenação SME'])) {
	...
	get after_get () {return __get__ (this, function (self, data, ajax_status) {
		...
		if (ajax_status == 'success') {
			...
			if (self.id_escola === null || self.id_escola === undefined) {
				...
				if (window.PhanterPWA.auth_user_has_role (['administrator', 'root', 'Administrador Master SME', 'Administrador Master Escola', 'Coordenação SME'])) {
...
export var EditarCalendario =  __class__ ('EditarCalendario', [object], {
	...
	get after_get () {return __get__ (this, function (self, data, ajax_status) {
		...
		if (ajax_status == 'success') {
			...
			self.data_final_ultima_unidade = json.data_final_ultima_unidade;
			if (self.id_escola === null || self.id_escola === undefined) {
				var logo_principal = '{0}/api/escolas/12/image'.format (window.PhanterPWA.ApiServer.remote_address);
	...
	get xml_modal_unidade () {return __get__ (this, function (self) {
		...
		for (var x = 0; x < self.quant_unidades; x++) {
			...
			if (__in__ (str (key), dict_periodo_unidades.py_keys ())) {
				...
				var final = TD (widgets.Inert ('df_unidade_{0}'.format (x + 1), __kwargtrans__ (dict ({'value': self.iso_br (dict_periodo_unidades [x + 1] [2])}))));
				if (x + 1 == self.quant_unidades) {
					var final = forms.FormWidget ('calendario_unidade', 'data_final_unidade_{0}'.format (x + 1), __kwargtrans__ (dict ({'value': self.data_final_ultima_unidade, 'label': 'Data Final', 'type': 'date', 'validators': ['IS_EMPTY_OR', 'IS_DATE:dd/MM/yyyy'], 'validator_format': '%d/%m/%Y', 'mask': 'dd/MM/yyyy', 'format': 'dd/MM/yyyy', '_placeholder': 'Data Inicial', '_class': 'p-col w1p100 input_data_unidade', '_data-unidade': 'final'})));
				}
				var linha = TR (TH ('Unidade {0}'.format (romanos [x])), TD (input_data), final);
			}
			else {
				if (x == 0) {
	...
	get _definir_unidade () {return __get__ (this, function (self, el) {
		...
		var formdata = new FormData ();
		if (unidade == 'final') {
			formdata.append ('data_final_ultima_unidade', nova_data);
		}
		else {
