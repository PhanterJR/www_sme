    ...
    def initialize(self):
        ...
        html = CONCATENATE(
            ...
            DIV(
                ...
                DIV(_id="float_button_container"),
...
class Diario():
    def __init__(self, index_instance, escola, ano_letivo, turma, id_matricula=None, eh_professor=False):
        ...
        if self.id_matricula is None:
            mat_link = ""
        else:
            mat_link = "/{0}".format(self.id_matricula)
        ...
        widgets.FloatMenu(
            ...
            widgets.FloatButton(
                ...
                _href="{0}/api/diario-de-notas/excel/{1}/{2}/{3}{4}/".format(
            ...
            widgets.FloatButton(
                ...
                _title="Salvar Diário de Notas como arquivo do excel",
                _class="botao_imprimir_diario_de_notas",
                _onclick="print();"
            ),
        ).html_to("#float_button_container")
        self._get_diario_de_notas()

    def processar_diario(self, diario_de_notas):
        html = DIV(
            H2("Diário de notas ", STRONG(diario_de_notas.turma.turma), _class="media-print-visible"),
            _class="diario_de_notas_container diario_de_notas_imprimir phanterpwa-simple-media-print"
        )
        ...
        for x in diario_de_notas.lista_alunos:
            ...
            if "selecionado" in x[1][1]["_class"]:
                add_class = "selecionado linha_aluno_abrir_diario"
            ...
            tabela_lista_alunos.append(
                XTRD(
                    "lista_alunos_diario_linha_{0}_{1}".format(self.id_turma, x[1][1]['_data-id_matricula']),
                    *[TD(y[0], **dict(y[1])) for y in x],
                    **{
                        "_class": add_class,
                        "_data-id_aluno": x[1][1]["_data-id_aluno"],
                        "_data-id_matricula": x[1][1]["_data-id_matricula"],
                        "drag_and_drop": False
                    }
                )
            )
        ...
        painel = DIV(
            ...
            DIV(
                ...
                tabela_lista_alunos,
                _class="painel_lista_turma e-padding_auto media-print-inside-invisible"
            ),
            _id="phanterpwa-card-panel-control-{0}".format(diario_de_notas.aluno.id),
            _class="phanterpwa-card-panel-control media-print-visible"
        )
    ...
    def diario_binds(self):
        ...
        if self.eh_professor:
            jQuery(
                ...
                "click.aluno_abrir_diario",
                lambda: self.abrir_diario(window.PhanterPWA.XWAY("diario-de-notas", "professor", self.id_escola, self.ano_letivo, self.id_turma, jQuery(this).data("id_matricula")))
            )
        else:
            jQuery(
                ".linha_aluno_abrir_diario"
                ...
                "click.aluno_abrir_diario",
                lambda: self.abrir_diario(window.PhanterPWA.XWAY("diario-de-notas", self.id_escola, self.ano_letivo, self.id_turma, jQuery(this).data("id_matricula")))
            )
    ...
    def _get_diario_de_notas(self):
        ...
        if self.id_matricula is not None and self.id_matricula is not js_undefined:
            if self.eh_professor:
