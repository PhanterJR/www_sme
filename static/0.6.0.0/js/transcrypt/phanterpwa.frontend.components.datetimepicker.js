// Transcrypt'ed from Python, 2022-04-26 16:30:42
...
export var Datepickers =  __class__ ('Datepickers', [object], {
	...
	get __init__ () {return __get__ (this, function (self, target_selector) {
		...
		if ($ (self.target_selector).length > 0) {
			if ($ (self.target_selector) [0].hasAttribute ('phanterpwa-datetimepicker-iso')) {
				...
				self.selected_date = new Date (phanterpwa_datetimepicker_iso.py_replace (' ', 'T'));
				self.current_date = new Date (self._apply_format ('yyyy-MM-01THH:ss:mm'));
			}
		}
	...
	get _onChoice () {return __get__ (this, function (self) {
		...
		if (callable (self.onChoice)) {
			var iso_format = self._apply_format ('yyyy-MM-dd HH:ss:mm');
			$ (self.target_selector).attr ('phanterpwa-datetimepicker-iso', iso_format);
			var data = dict ([['iso', iso_format], ['formated', self._apply_format (self.format)]]);
			self.onChoice (data);
		}
		else {
			var iso_format = self._apply_format ('yyyy-MM-dd HH:ss:mm');
			$ (self.target_selector).attr ('phanterpwa-datetimepicker-iso', iso_format);
			if ($ (self.id_input_target).length > 0) {
				$ (self.id_input_target).val (self._apply_format (self.format)).focus ().trigger ('change');
			}
			if (self.debug) {
	...
	get _read_formated () {return __get__ (this, function (self, value) {
		...
		self.selected_date = new Date ('{0}-{1}-{2}T{3}:{4}:{5}'.format (year, month, day, hour, minute, second));
		self.current_date = new Date ('{0}-{1}-01T{2}:{3}:{4}'.format (year, month, hour, minute, second));
	});},
	...
	get _selected_date () {return __get__ (this, function (self, value) {
		...
		if (self.date_type == 'date') {
			...
			if (REGEX_BODY.test (value)) {
				self.selected_date = new Date ('{0}T00:00:00'.format (value.__getslice__ (0, 10, 1)));
				self.current_date = new Date ('{0}01T00:00:00'.format (value.__getslice__ (0, 8, 1)));
			}
			else {
			...
			if (REGEX_BODY.test (value)) {
				self.selected_date = new Date ('{0}T{1}'.format (value.__getslice__ (0, 10, 1), value.__getslice__ (11, 19, 1)));
				self.current_date = new Date ('{0}01T{1}'.format (value.__getslice__ (0, 8, 1), value.__getslice__ (11, 19, 1)));
			}
			else {
				var __except0__ = ValueError ("The current_date must be 'yyyy-mm-dd'. Example: 2019-01-01");
				__except0__.__cause__ = null;
	...
	get previus_year () {return __get__ (this, function (self) {
		...
		var new_date = '{0}-{1}-{2}T{3}:{4}:{5}'.format (cyear - 1, self._zfill (cmonth + 1, 2), self._zfill (cday, 2), self._zfill (chour, 2), self._zfill (cminute, 2), self._zfill (csecond, 2));
	...
	get previus_month () {return __get__ (this, function (self) {
		...
		var new_date = '{0}-{1}-{2}T{3}:{4}:{5}'.format (cyear, self._zfill (calc_month + 1, 2), self._zfill (1, 2), self._zfill (chour, 2), self._zfill (cminute, 2), self._zfill (csecond, 2));
		self.current_date = new Date (new_date);
		self.start ();
	});},
	get next_month () {return __get__ (this, function (self) {
		...
		var snew_date = '{0}T{1}:{2}:{3}'.format (sdate.__getslice__ (0, 10, 1), self._zfill (chour, 2), self._zfill (cminute, 2), self._zfill (csecond, 2));
	...
	get previus_hour () {return __get__ (this, function (self) {
		...
		var sdate = self.selected_date.toJSON ();
		var snew_date = '{0}T{1}:{2}:{3}'.format (sdate.__getslice__ (0, 10, 1), self._zfill (calc_hour, 2), self._zfill (cminute, 2), self._zfill (csecond, 2));
		self.selected_date = new Date (snew_date);
		var new_date = '{0}-{1}-{2}T{3}:{4}:{5}'.format (cyear, self._zfill (cmonth + 1, 2), self._zfill (1, 2), self._zfill (calc_hour, 2), self._zfill (cminute, 2), self._zfill (csecond, 2));
		self.current_date = new Date (new_date);
		self.start ();
	});},
	...
	get previus_minute () {return __get__ (this, function (self) {
		...
		var sdate = self.selected_date.toJSON ();
		var snew_date = '{0}T{1}:{2}:{3}'.format (sdate.__getslice__ (0, 10, 1), self._zfill (chour, 2), self._zfill (calc_minute, 2), self._zfill (csecond, 2));
		self.selected_date = new Date (snew_date);
		var new_date = '{0}-{1}-{2}T{3}:{4}:{5}'.format (cyear, self._zfill (cmonth + 1, 2), self._zfill (1, 2), self._zfill (chour, 2), self._zfill (calc_minute, 2), self._zfill (csecond, 2));
		self.current_date = new Date (new_date);
		self.start ();
	});},
	...
	get previus_second () {return __get__ (this, function (self) {
		...
		var sdate = self.selected_date.toJSON ();
		var snew_date = '{0}T{1}:{2}:{3}'.format (sdate.__getslice__ (0, 10, 1), self._zfill (chour, 2), self._zfill (cminute, 2), self._zfill (calc_second, 2));
		self.selected_date = new Date (snew_date);
		var new_date = '{0}-{1}-{2}T{3}:{4}:{5}'.format (cyear, self._zfill (cmonth + 1, 2), self._zfill (1, 2), self._zfill (chour, 2), self._zfill (cminute, 2), self._zfill (calc_second, 2));
		self.current_date = new Date (new_date);
		self.start ();
	});},
	...
	get set_year () {return __get__ (this, function (self, value) {
		...
		if (REGEX_BODY.test (value)) {
			var cmonth = self.current_date.getMonth ();
			var new_date = '{0}-{1}-{2}T{3}:{4}:{5}'.format (value, self._zfill (cmonth + 1, 2), self._zfill (1, 2), self._zfill (chour, 2), self._zfill (cminute, 2), self._zfill (csecond, 2));
			self.current_date = new Date (new_date);
			self.start ();
		}
	});},
	get set_month () {return __get__ (this, function (self, value) {
		...
		var csecond = self.current_date.getSeconds ();
		var cyear = self.current_date.getFullYear ();
		var new_date = '{0}-{1}-{2}T{3}:{4}:{5}'.format (cyear, self._zfill (int (value) + 1, 2), self._zfill (1, 2), self._zfill (chour, 2), self._zfill (cminute, 2), self._zfill (csecond, 2));
		self.current_date = new Date (new_date);
		self.start ();
	});},
	get set_hour () {return __get__ (this, function (self, value) {
		...
		var csecond = self.current_date.getSeconds ();
		var sdate = self.selected_date.toJSON ();
		var snew_date = '{0}T{1}:{2}:{3}'.format (sdate.__getslice__ (0, 10, 1), self._zfill (int (value), 2), self._zfill (cminute, 2), self._zfill (csecond, 2));
		self.selected_date = new Date (snew_date);
		var new_date = '{0}-{1}-{2}T{3}:{4}:{5}'.format (cyear, self._zfill (cmonth + 1, 2), self._zfill (1, 2), self._zfill (int (value), 2), self._zfill (cminute, 2), self._zfill (csecond, 2));
		self.current_date = new Date (new_date);
		self.start ();
	});},
	get set_minute () {return __get__ (this, function (self, value) {
		...
		var csecond = self.current_date.getSeconds ();
		var sdate = self.selected_date.toJSON ();
		var snew_date = '{0}T{1}:{2}:{3}'.format (sdate.__getslice__ (0, 10, 1), self._zfill (chour, 2), self._zfill (value, 2), self._zfill (csecond, 2));
		self.selected_date = new Date (snew_date);
		var new_date = '{0}-{1}-{2}T{3}:{4}:{5}'.format (cyear, self._zfill (cmonth + 1, 2), self._zfill (1, 2), self._zfill (chour, 2), self._zfill (value, 2), self._zfill (csecond, 2));
		self.current_date = new Date (new_date);
		self.start ();
	});},
	get set_second () {return __get__ (this, function (self, value) {
		...
		var cminute = self.current_date.getMinutes ();
		var sdate = self.selected_date.toJSON ();
		var snew_date = '{0}T{1}:{2}:{3}'.format (sdate.__getslice__ (0, 10, 1), self._zfill (chour, 2), self._zfill (cminute, 2), self._zfill (value, 2));
		self.selected_date = new Date (snew_date);
		var new_date = '{0}-{1}-{2}T{3}:{4}:{5}'.format (cyear, self._zfill (cmonth + 1, 2), self._zfill (1, 2), self._zfill (chour, 2), self._zfill (cminute, 2), self._zfill (value, 2));
		self.current_date = new Date (new_date);
		self.start ();
	});},
	get set_selected () {return __get__ (this, function (self, jsonDate) {
		...
		var new_date = new Date (jsonDate.py_replace (' ', 'T'));
	...
	get start () {return __get__ (this, function (self) {
		...
		for (var x = 0; x < 40; x++) {
			if (inicial_day < 32) {
				cont += 1;
				var new_date = '{0}-{1}-{2}T{3}:{4}:{5}'.format (cyear, self._zfill (cmonth + 1, 2), self._zfill (inicial_day, 2), self._zfill (chour, 2), self._zfill (cminute, 2), self._zfill (csecond, 2));
		...
		var centralizer = DIV (DIV (DIV (DIV (DIV (datetimepicker_container, __kwargtrans__ ({_id: 'phanterpwa-centralizer-center-{0}'.format (self.namespace), _class: 'phanterpwa-centralizer-center'})), __kwargtrans__ ({_class: 'phanterpwa-centralizer-horizontal'})), __kwargtrans__ ({_class: 'phanterpwa-centralizer-vertical'})), __kwargtrans__ ({_class: 'phanterpwa-centralizer-wrapper'})), __kwargtrans__ ({_class: 'phanterpwa-fixed-fulldisplay', _style: 'z-index: 1008;'}));
