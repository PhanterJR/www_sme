// Transcrypt'ed from Python, 2022-04-26 16:30:38
...
export var Handler =  __class__ ('Handler', [object], {
	...
	get set_title () {return __get__ (this, function (self, value) {
		...
		if (value === null) {
			...
			if (window.PhanterPWA.CONFIG.PROJECT.title != title) {
				$ ('title').text (window.PhanterPWA.CONFIG.PROJECT.title);
			}
		}
		else {
			$ ('title').text (value);
...
export var ErrorHandler =  __class__ ('ErrorHandler', [object], {
	__module__: __name__,
	get __init__ () {return __get__ (this, function (self, request, response, reasons) {
...
export var Error_401 =  __class__ ('Error_401', [ErrorHandler], {
	...
	get start () {return __get__ (this, function (self) {
		...
		var html = CONCATENATE (DIV (DIV (DIV (DIV (I18N ('Authentication required', __kwargtrans__ (dict ([['_pt-br', 'Necessário Autenticar-se']]))), __kwargtrans__ ({_class: 'phanterpwa-breadcrumb'})), __kwargtrans__ ({_class: 'phanterpwa-breadcrumb-wrapper'})), __kwargtrans__ ({_class: 'phanterpwa-container container'})), __kwargtrans__ ({_class: 'title_page_container card'})), DIV (DIV (DIV (DIV (DIV (IMG (__kwargtrans__ ({_class: 'image-warnings', _src: '/static/{0}/images/warning.png'.format (window.PhanterPWA.VERSIONING)}))), __kwargtrans__ ({_class: 'image-warnings-container'})), DIV (H3 (I18N ('You need authentication to access this feature.', __kwargtrans__ (dict ([['_pt-br', 'Você precisa autenticar-se para tentar acessar este recurso.']])))), (self.reasons !== null ? H3 (self.reasons) : ''), __kwargtrans__ ({_id: 'content-warning', _class: 'content-warnings'})), DIV (DIV (I18N ('Login'), __kwargtrans__ ({_id: 'alternative_login_button', _class: 'btn wave_on_click link'})), __kwargtrans__ ({_class: 'button-container'})), __kwargtrans__ ({_class: 'warnings-container card phanterpwa-card-container'})), __kwargtrans__ ({_class: 'new-container'})), __kwargtrans__ ({_class: 'phanterpwa-container container'})));
...
export var Error_403 =  __class__ ('Error_403', [ErrorHandler], {
	...
	get start () {return __get__ (this, function (self) {
		...
		html.find ('.image-warnings').attr ('src', '/static/{0}/images/warning.png'.format (window.PhanterPWA.VERSIONING));
		var html_warning = CONCATENATE (H3 ('ERROR 403 - Forbidden'), (self.reasons !== null ? P (self.reasons) : ''));
...
export var Error_404 =  __class__ ('Error_404', [ErrorHandler], {
	...
	get start () {return __get__ (this, function (self) {
		...
		var html = html_base.jquery ();
		html.find ('.image-warnings').attr ('src', '/static/{0}/images/warning.png'.format (window.PhanterPWA.VERSIONING));
		var html_warning = CONCATENATE (H3 ('ERROR 404 - Not Found'), (self.reasons !== null ? P (self.reasons) : ''));
		html.find ('#content-warning').html (html_warning.jquery ());
		$ ('#main-container').html (html);
	});}
});
export var Error_409 =  __class__ ('Error_409', [ErrorHandler], {
	__module__: __name__,
	get start () {return __get__ (this, function (self) {
		if (arguments.length) {
			var __ilastarg0__ = arguments.length - 1;
...
export var Error_500 =  __class__ ('Error_500', [ErrorHandler], {
	...
	get start () {return __get__ (this, function (self) {
		...
		var html = html_base.jquery ();
		html.find ('.image-warnings').attr ('src', '/static/{0}/images/warning.png'.format (window.PhanterPWA.VERSIONING));
		var html_warning = CONCATENATE (H3 ('ERROR 500 - Internal Error'), (self.reasons !== null ? P (self.reasons) : ''));
		html.find ('#content-warning').html (html_warning.jquery ());
		$ ('#main-container').html (html);
	});}
});
export var Error_502 =  __class__ ('Error_502', [ErrorHandler], {
	...
	get start () {return __get__ (this, function (self) {
		...
		var html = html_base.jquery ();
		html.find ('.image-warnings').attr ('src', '/static/{0}/images/warning.png'.format (window.PhanterPWA.VERSIONING));
		var html_warning = CONCATENATE (H3 ('ERROR 502 - Bad Gateway'), (self.reasons !== null ? P (self.reasons) : ''));
		html.find ('#content-warning').html (html_warning.jquery ());
		$ ('#main-container').html (html);
	});}
});
