// Transcrypt'ed from Python, 2021-10-08 10:21:20
...
export var Index =  __class__ ('Index', [gatehandler.Handler], {
	...
	get initialize () {return __get__ (this, decorators.check_authorization ((function __lambda__ () {
		...
		if (str (arg0).isdigit () && str (arg1).isdigit () && str (arg2).isdigit () && (arg3 === null || arg3 === undefined)) {
			var html = CONCATENATE (DIV (DIV (DIV (DIV ('CONSELHO DE CLASSE', __kwargtrans__ ({_class: 'phanterpwa-breadcrumb'})), tur, DIV ('QUAL A UNIDADE?', __kwargtrans__ ({_class: 'phanterpwa-breadcrumb'})), __kwargtrans__ ({_class: 'phanterpwa-breadcrumb-wrapper'})), __kwargtrans__ ({_class: 'p-container extend'})), __kwargtrans__ ({_class: 'title_page_container card'})), DIV (DIV (self.xml_unidades (), __kwargtrans__ ({_id: 'content-conselho_de_classe', _class: 'p-row card e-padding_auto'})), __kwargtrans__ ({_class: 'phanterpwa-container p-container extend'})));
			html.html_to ('#main-container');
		}
		else if (str (arg0).isdigit () && str (arg1).isdigit () && str (arg2).isdigit () && str (arg3).isdigit ()) {
	...
	get xml_unidades () {return __get__ (this, function (self) {
		...
		return DIV (DIV ('QUAL UNIDADE FARÁ O CONSELHO?', __kwargtrans__ ({_class: 'card-title'})), html);
	});}
});
export var ConselhoDeClasse =  __class__ ('ConselhoDeClasse', [object], {
	...
	get after_get () {return __get__ (this, function (self, data, ajax_status) {
		...
		if (ajax_status == 'success') {
			...
			var conselho_de_classe = json.conselho_de_classe;
			self.disciplinas = json.disciplinas;
			self.turma = json.turma;
			self.processar_conselho (conselho_de_classe);
		}
	});},
	get processar_conselho () {return __get__ (this, function (self, conselho_de_classe) {
		if (arguments.length) {
			var __ilastarg0__ = arguments.length - 1;
			if (arguments [__ilastarg0__] && arguments [__ilastarg0__].hasOwnProperty ("__kwargtrans__")) {
				var __allkwargs0__ = arguments [__ilastarg0__--];
				for (var __attrib0__ in __allkwargs0__) {
					switch (__attrib0__) {
						case 'self': var self = __allkwargs0__ [__attrib0__]; break;
						case 'conselho_de_classe': var conselho_de_classe = __allkwargs0__ [__attrib0__]; break;
					}
				}
			}
		}
		...
		html.html_to ('#main-container');
		var tabela = TABLE (TR (TH ('ALUNO'), TH ('DISCIPLINA'), TH ('AV1'), TH ('AV2'), TH ('AV3'), TH ('AV4'), TH ('AV5'), TH ('MÉDIA')), __kwargtrans__ ({_class: 'tabela_conselho_de_classe', _style: 'margin: auto; width: 200mm;'}));
		for (var x of conselho_de_classe) {
