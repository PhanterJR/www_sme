// Transcrypt'ed from Python, 2022-01-19 13:43:07
...
export var TurmasSimples =  __class__ ('TurmasSimples', [helpers.XmlConstructor], {
	...
	get turmas_disponiveis () {return __get__ (this, function (self) {
		...
		var aviso_temporario = '';
		if (self.tem_multisseriado) {
			var aviso_temporario = H2 ('Professor(a), a sua turma multisseriada está passando por um processo de mudança no registro de aulas. Tentarei adaptar as aulas já registradas para o novo modelo. Este processo é totalmente reversível caso não dê certo. Evite alterar registros feitos anteriormente por enquanto, mas você pode tentar fazer novos registros para tentar se adaptar ao novo modelo. Agradeçemos a sua compreensão.', __kwargtrans__ ({_style: 'color: red'}));
		}
		aviso_temporario;
		var html = DIV (DIV (DIV (DIV (IMG (__kwargtrans__ ({_src: logo})), __kwargtrans__ ({_class: 'escolas-container-info-image', _style: 'text-align: center;'})), __kwargtrans__ ({_class: 'p-col w1p100 w4p30'})), DIV (H2 ('NESTE PAINEL É POSSÍVEL VISUALIZAR TODAS AS TURMAS ÀS QUAIS ENSINA.'), P ('Acessando o botão ', I (__kwargtrans__ ({_class: 'fas fa-ellipsis-v'})), ' é possível acessar opções', ' relacionadas à referente turma, como: ', STRONG ('Diário de Notas, Ficha Avaliativa,', ' Registro de Faltas, Registro de Aulas, etc.')), P ('Se estiver faltando alguma turma ou disciplina, informe a direção de sua escola.'), __kwargtrans__ ({_class: 'p-col w1p100 w4p70', _id: 'u_informacao_series'})), __kwargtrans__ ({_class: 'p-row'})), DIV (aviso_temporario, self.xml_tabela_turmas (), DIV (__kwargtrans__ ({_id: 'modal_turma_case'})), __kwargtrans__ ({_class: 'p-row'})), __kwargtrans__ ({_class: 'turmas-turmas-container'}));
