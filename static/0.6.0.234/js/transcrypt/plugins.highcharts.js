// Transcrypt'ed from Python, 2022-01-19 13:43:10
...
export var BarStacked =  __class__ ('BarStacked', [widgets.Widget], {
	__module__: __name__,
	get __init__ () {return __get__ (this, function (self, identifier) {
		var parameters = dict ();
		if (arguments.length) {
			...
			var categories = tuple ([].slice.apply (arguments).slice (2, __ilastarg0__ + 1));
		}
		else {
			var categories = tuple ();
		}
		self.categories = categories;
		parameters ['_id'] = identifier;
		if (__in__ ('_class', parameters)) {
			parameters ['_class'] = '{0}{1}'.format (parameters ['_class'].strip (), ' phanterpwa-plugin-highcharts-barstacked');
		}
		else {
	...
	get _series_validator () {return __get__ (this, function (self) {
		...
		for (var x of self._series) {
			...
			if (!__in__ (field, ordem)) {
				ordem.append (field);
				var value = x ['values'];
				new_serie [field] = dict ({'name': field, 'data': value});
				if (__in__ ('color', x)) {
					new_serie [field] ['color'] = x ['color'];
				}
			}
	...
	get add_serie () {return __get__ (this, function (self, field_name) {
		var kargs = dict ();
		if (arguments.length) {
			var __ilastarg0__ = arguments.length - 1;
			if (arguments [__ilastarg0__] && arguments [__ilastarg0__].hasOwnProperty ("__kwargtrans__")) {
			...
			var py_values = tuple ([].slice.apply (arguments).slice (2, __ilastarg0__ + 1));
		}
		else {
			var py_values = tuple ();
		}
		if (len (py_values) == len (self.categories) && field_name) {
			if (field_name) {
				var p = dict (kargs);
				p ['field'] = field_name;
				p ['values'] = (function () {
					var __accu0__ = [];
					for (var x of py_values) {
						__accu0__.append (float (x));
					}
					return __accu0__;
				}) ();
				self._series.append (p);
	...
	get start () {return __get__ (this, function (self) {
		...
		self._series_validator ();
		Highcharts.chart ('{0}'.format (self._chart_target), dict ({'chart': dict ({'plotBackgroundColor': null, 'plotBorderWidth': null, 'plotShadow': false, 'type': 'bar'}), 'title': dict ({'text': self.title}), 'tooltip': dict ({'pointFormat': self.pointFormat}), 'xAxis': dict ({'categories': self.categories}), 'yAxis': dict ({'min': 0, 'title': dict ({'text': self.series_name})}), 'credits': false, 'plotOptions': dict ({'series': dict ({'stacking': 'normal', 'dataLabels': dict ({'enabled': self.showLabels}), 'showInLegend': self.showLegend})}), 'series': self.data_serie}));
	});}
});
