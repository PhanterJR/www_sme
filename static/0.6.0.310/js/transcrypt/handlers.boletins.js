// Transcrypt'ed from Python, 2022-01-19 13:42:51
...
var __name__ = 'handlers.boletins';
...
export var Index =  __class__ ('Index', [gatehandler.Handler], {
	...
	get initialize () {return __get__ (this, decorators.check_authorization ((function __lambda__ () {
		...
		var html = CONCATENATE (DIV (DIV (DIV (DIV ('BOLETINS', __kwargtrans__ ({_class: 'phanterpwa-breadcrumb'})), tur, __kwargtrans__ ({_class: 'phanterpwa-breadcrumb-wrapper'})), __kwargtrans__ ({_class: 'p-container extend'})), __kwargtrans__ ({_class: 'title_page_container card'})), DIV (DIV (DIV (DIV (preloaders.android, __kwargtrans__ ({_style: 'width: 300px; height: 300px; overflow: hidden; margin: auto;'})), __kwargtrans__ ({_style: 'text-align:center; padding: 50px 0;'})), __kwargtrans__ ({_id: 'content-boletins', _class: 'p-row card e-padding_auto'})), __kwargtrans__ ({_class: 'phanterpwa-container p-container extend'})));
		if (str (arg0).isdigit () && str (arg1).isdigit () && str (arg2).isdigit ()) {
			self.Boletins = Boletins (self, arg0, arg1, arg2);
		}
		var BackButton1 = left_bar.LeftBarButton ('back_boletins', 'Voltar', I (__kwargtrans__ ({_class: 'fas fa-arrow-circle-left'})), __kwargtrans__ (dict ({'tag': 'a', '_href': window.PhanterPWA.Request.get_param ('retornar'), 'position': 'top', 'show_if_way_match': '^boletins\\/[0-9]{1,}\\/[0-9]{1,}.*$'})));
		window.PhanterPWA.Components ['left_bar'].add_button (BackButton1);
	}));}
});
export var Boletins =  __class__ ('Boletins', [object], {
	__module__: __name__,
	get __init__ () {return __get__ (this, function (self, index_instance, escola, ano_letivo, turma) {
		if (arguments.length) {
			var __ilastarg0__ = arguments.length - 1;
			if (arguments [__ilastarg0__] && arguments [__ilastarg0__].hasOwnProperty ("__kwargtrans__")) {
				var __allkwargs0__ = arguments [__ilastarg0__--];
		...
		var BackButton1 = left_bar.LeftBarButton ('back_boletins', 'Voltar', I (__kwargtrans__ ({_class: 'fas fa-arrow-circle-left'})), __kwargtrans__ (dict ({'tag': 'a', '_href': window.PhanterPWA.XWAY ('turma-simples', self.id_escola, self.ano_letivo), 'position': 'top', 'show_if_way_match': '^boletins\\/[0-9]{1,}\\/[0-9]{1,}.*$'})));
		window.PhanterPWA.Components ['left_bar'].add_button (BackButton1);
	});},
	get _get_dados_conselho () {return __get__ (this, function (self) {
		...
		window.PhanterPWA.GET ('api', 'boletins', self.id_escola, self.ano_letivo, self.id_turma, __kwargtrans__ ({onComplete: self.after_get}));
	});},
	get after_get () {return __get__ (this, function (self, data, ajax_status) {
		...
		if (ajax_status == 'success') {
			...
			var boletins = json.data.boletins;
			self.turma = json.data.turma;
			self.processar_boletins (boletins);
		}
	});},
	get processar_boletins () {return __get__ (this, function (self, boletins) {
		if (arguments.length) {
			var __ilastarg0__ = arguments.length - 1;
			if (arguments [__ilastarg0__] && arguments [__ilastarg0__].hasOwnProperty ("__kwargtrans__")) {
				var __allkwargs0__ = arguments [__ilastarg0__--];
				for (var __attrib0__ in __allkwargs0__) {
					switch (__attrib0__) {
						case 'self': var self = __allkwargs0__ [__attrib0__]; break;
						case 'boletins': var boletins = __allkwargs0__ [__attrib0__]; break;
					}
				}
			}
		}
		...
		if (boletins !== null && boletins !== undefined) {
			var cont = 0;
			for (var x of boletins) {
				cont++;
				var _class = '';
				if (__mod__ (cont, 4)) {
					var _class = ' quebra-linha';
				}
				var html_ficha = P ('Boletin não encontrado no ano letivo de ', STRONG (x.ano_letivo), '.');
				if (x.boletin !== null && x.boletin !== undefined) {
					var tabela_rotulo = TABLE (TR (TH ('Aluno(a):'), TD (x.aluno, __kwargtrans__ ({_colspan: 4}))), TR (TH ('Turma:'), TD (x.turma), TH ('Série:'), TD (x.serie)), __kwargtrans__ ({_class: 'tabela_ficha_individual', _style: 'width: 100%; margin-bottom: 5px;'}));
					var tabela = TABLE (__kwargtrans__ ({_class: 'tabela_ficha_individual p-row'}));
					for (var y of x.boletin) {
						var linha = TR ();
						var eh_desistente = false;
						var eh_transferido = false;
						for (var c of y) {
							...
							else if (c [0] == 'TRANSFERIDO(A)' && !(eh_transferido)) {
								var eh_transferido = true;
								linha.append (TD (c [0], __kwargtrans__ (__mergekwargtrans__ ({_colspan: len (y)}, dict (c [1])))));
							}
							else if (c [0] == 'DESISTENTE' && !(eh_desistente)) {
								var eh_desistente = true;
								linha.append (TD (c [0], __kwargtrans__ (__mergekwargtrans__ ({_colspan: len (y)}, dict (c [1])))));
							}
							else if (c [0] == 'DESISTENTE' || c [0] == 'TRANSFERIDO(A)') {
								// pass;
							}
							else {
	...
	get binds () {return __get__ (this, function (self) {
		...
		$ ('.exibir_media_final_switch').off ('click.medfin').on ('click.medfin', (function __lambda__ () {
			if (arguments.length) {
				var __ilastarg0__ = arguments.length - 1;
				if (arguments [__ilastarg0__] && arguments [__ilastarg0__].hasOwnProperty ("__kwargtrans__")) {
					var __allkwargs0__ = arguments [__ilastarg0__--];
			...
			return self.ocultar_mostrar_media_e_rec_final (this);
		}));
	});},
	get ocultar_mostrar_media_e_rec_final () {return __get__ (this, function (self, el) {
		if (arguments.length) {
			var __ilastarg0__ = arguments.length - 1;
			if (arguments [__ilastarg0__] && arguments [__ilastarg0__].hasOwnProperty ("__kwargtrans__")) {
				var __allkwargs0__ = arguments [__ilastarg0__--];
		...
		if (element.hasClass ('actived')) {
			...
			$ ('#content-boletins').removeClass ('ocultar_medias');
		}
		else {
			element.addClass ('actived');
			$ ('#content-boletins').addClass ('ocultar_medias');
		}
	});}
});

//# sourceMappingURL=handlers.boletins.map