// Transcrypt'ed from Python, 2022-03-14 09:40:01
...
export var Disciplinas =  __class__ ('Disciplinas', [helpers.XmlConstructor], {
	...
	get escolha_de_disciplinas_xml () {return __get__ (this, function (self) {
		...
		var html = DIV (DIV (DIV (DIV (IMG (__kwargtrans__ ({_src: logo})), __kwargtrans__ ({_class: 'escolas-container-info-image', _style: 'text-align: center;'})), __kwargtrans__ ({_class: 'p-col w1p100 w4p30'})), DIV (H2 ('ADICIONE EM CADA SÉRIE AS DISCIPLINAS OFERTADAS PELA SUA UNIDADE DE ENSINO', ' NO ANO LETIVO DE ', STRONG (self.ano_letivo)), P ('Para adicionar uma nova disciplina, basta clicar no botão ', I (__kwargtrans__ ({_class: 'fas fa-plus'})), '. Para mudar a ordem basta arrastar a disciplina na posição desejada.'), P ('Muitas séries possuem disciplinas idênticas, você pode copiar e colar todas as disciplinas', ' utilizando os botões ', I (__kwargtrans__ ({_class: 'fas fa-copy'})), ' (para copiar) e ', I (__kwargtrans__ ({_class: 'fas fa-paste'})), ' (para colar).'), __kwargtrans__ ({_class: 'p-col w1p100 w4p70', _id: 'u_informacao_disciplinas'})), __kwargtrans__ ({_class: 'p-row'})), __kwargtrans__ ({_class: 'series-disciplinas-container'}));
	...
	get xml_card () {return __get__ (this, function (self, data_serie) {
		...
		var table = XTABLE ('disciplinas-table', XTRH ('disciplinas-table-head', 'Disciplina', TH ('Abreviação', __kwargtrans__ ({_style: 'text-align: center;'})), TH ('Carga Horária', __kwargtrans__ ({_style: 'text-align: center;'})), TH ('Diário de Notas?', __kwargtrans__ ({_style: 'text-align: center;'})), TH ('Base Nacional Comum?', __kwargtrans__ ({_style: 'text-align: center;'})), TH (__kwargtrans__ ({_style: 'width: 40px;'}))));
		var cont_disc = 0;
		...
		if (cont_disc == 0) {
			var table = DIV ('NÃO HÁ DISCIPLINAS CADASTRADAS NESTA TURMA', __kwargtrans__ ({_style: 'color: red; padding: 50px 0; text-align: center;'}));
		}
