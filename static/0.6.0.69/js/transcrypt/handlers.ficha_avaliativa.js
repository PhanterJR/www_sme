...
export var FichaAvaliativa =  __class__ ('FichaAvaliativa', [object], {
	...
	get processar_diario () {return __get__ (this, function (self, diario_de_notas, professor, disciplina, turma) {
		...
		var html = DIV (H2 ('Ficha Avaliativa ', STRONG (turma)), DIV (DIV (professor, __kwargtrans__ ({_class: 'p-col w1p100 w4p50'})), DIV (disciplina, __kwargtrans__ ({_class: 'p-col w1p100 w4p50'})), __kwargtrans__ ({_class: 'p-row'})), HR (), __kwargtrans__ ({_class: 'diario_de_notas_container'}));
		...
		for (var x of diario_de_notas) {
			...
			for (var y of x) {
				if (y [1] ['tipo'] == 'cabecalho') {
					...
					var celula = TD (DIV (INPUT (__kwargtrans__ ({_value: y [0]})), __kwargtrans__ ({_class: 'ficha_avaliativa_content_input_textarea'})), DIV (I (__kwargtrans__ ({_class: 'fas fa-cloud-upload-alt'})), __kwargtrans__ ({_class: 'botao_enviar_nota'})), __kwargtrans__ (y [1]));
				}
		...
		html.append (DIV (DIV (tabela, __kwargtrans__ ({_class: class_ac})), __kwargtrans__ ({_class: 'diario-ficha_avaliativa-wrapper'})));
	...
	get after_get () {return __get__ (this, function (self, data, ajax_status) {
		...
		if (ajax_status == 'success') {
			...
			if (diario_de_notas.disciplina !== null && diario_de_notas.disciplina !== undefined) {
				var disciplina = DIV (STRONG ('Disciplina'), SPAN (diario_de_notas.disciplina), __kwargtrans__ ({_class: 'e-tagger-wrapper'}));
			}
	...
	get onKeyPress () {return __get__ (this, function (self, event, el) {
		...
		var element = $ (el);
		if (code == 13) {
			var proximo_campo = element.parent ().parent ().data ('proximo_campo');
