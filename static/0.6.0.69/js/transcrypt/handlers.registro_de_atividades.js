// Transcrypt'ed from Python, 2021-09-23 20:37:18
...
export var Index =  __class__ ('Index', [gatehandler.Handler], {
	...
	get initialize () {return __get__ (this, decorators.check_authorization ((function __lambda__ () {
		...
		if (window.PhanterPWA.Request.get_param ('retornar') === null) {
			var BackButton1 = left_bar.LeftBarButton ('back_controle_de_atividades', 'Voltar', I (__kwargtrans__ ({_class: 'fas fa-arrow-circle-left'})), __kwargtrans__ (dict ({'tag': 'a', '_href': window.PhanterPWA.XWAY ('professores', 'turmas', arg0, arg1), 'position': 'top', 'show_if_way_match': '^registro-de-atividades\\/[0-9]{1,}\\/[0-9]{1,}.*$'})));
		}
		else if (window.PhanterPWA.Request.get_param ('retornar') == 'turma-simples') {
			var BackButton1 = left_bar.LeftBarButton ('back_controle_de_atividades', 'Voltar', I (__kwargtrans__ ({_class: 'fas fa-arrow-circle-left'})), __kwargtrans__ (dict ({'tag': 'a', '_href': window.PhanterPWA.XWAY ('turmas', arg0, arg1), 'position': 'top', 'show_if_way_match': '^registro-de-atividades\\/[0-9]{1,}\\/[0-9]{1,}.*$'})));
		}
...
export var RegistroDeAtividades =  __class__ ('RegistroDeAtividades', [object], {
	...
	get _get_controle_de_atividades () {return __get__ (this, function (self) {
		...
		if (self.id_disciplina !== null && self.id_disciplina !== undefined) {
			if (self.rotulo_mes_ano !== null && self.rotulo_mes_ano !== undefined) {
				window.PhanterPWA.GET ('api', 'registro-de-atividades', self.id_escola, self.ano_letivo, self.id_turma, self.id_disciplina, self.rotulo_mes_ano, __kwargtrans__ ({onComplete: self.after_get}));
			}
			else {
				window.PhanterPWA.GET ('api', 'registro-de-atividades', self.id_escola, self.ano_letivo, self.id_turma, self.id_disciplina, __kwargtrans__ ({onComplete: self.after_get}));
			}
		}
		else {
	...
	get processar_controle_de_atividades () {return __get__ (this, function (self, controle_de_atividades) {
		...
		if (self.eh_educacao_infantil) {
			...
			var tabela = TABLE (TR (TH ('Nº', __kwargtrans__ ({_class: 'rotulo', _rowspan: 2})), TH ('NOME ALUNO(A)', __kwargtrans__ ({_class: 'rotulo', _rowspan: 2})), TH ('DISCIPLINAS', __kwargtrans__ ({_class: 'rotulo', _rowspan: 2})), TH ('DIAS LETIVOS', __kwargtrans__ ({_colspan: self.quant_dias, _class: 'rotulo'}))), __kwargtrans__ ({_class: 'tabela_controle_de_atividades'}));
		}
