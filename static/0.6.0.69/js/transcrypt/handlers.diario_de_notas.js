// Transcrypt'ed from Python, 2021-08-26 06:02:44
...
export var Diario =  __class__ ('Diario', [object], {
	__module__: __name__,
	get __init__ () {return __get__ (this, function (self, index_instance, escola, ano_letivo, turma, id_aluno, eh_professor) {
	...
	get processar_diario () {return __get__ (this, function (self, diario_de_notas) {
		...
		var html = DIV (H2 ('Diário de notas ', STRONG (diario_de_notas.turma.turma)), __kwargtrans__ ({_class: 'diario_de_notas_container'}));
		...
		for (var x of diario_de_notas.lista_alunos) {
			...
			if (__in__ ('selecionado', x [1] [1] ['_class'])) {
				var add_class = 'link selecionado linha_aluno_abrir_diario';
			}
			tabela_lista_alunos.append (XTRD ('lista_alunos_diario_linha_{0}_{1}'.format (self.id_turma, x [1] [1] ['_data-id_aluno']), ...(function () {
				var __accu0__ = [];
				for (var y of x) {
					__accu0__.append (TD (y [0], __kwargtrans__ (dict (y [1]))));
				}
				return __accu0__;
			}) (), __kwargtrans__ (dict ({'_class': add_class, '_data-id_aluno': x [1] [1] ['_data-id_aluno'], 'drag_and_drop': false}))));
		}
		var botao_aluno_anterior = DIV (I (__kwargtrans__ ({_class: 'fas fa-angle-double-left'})), __kwargtrans__ (dict ({'_class': 'botao_aluno_anterior icon_button', '_title': 'Diário do Aluno Anterior', '_disabled': 'disabled'})));
		var botao_proximo_aluno = DIV (I (__kwargtrans__ ({_class: 'fas fa-angle-double-right'})), __kwargtrans__ (dict ({'_class': 'botao_proximo_aluno icon_button', '_title': 'Diário do Próximo Aluno turma', '_disabled': 'disabled'})));
		if (self.eh_professor) {
	...
	get diario_binds () {return __get__ (this, function (self) {
		...
		if (self.eh_professor) {
			$ ('.linha_aluno_abrir_diario').off ('click.aluno_abrir_diario').on ('click.aluno_abrir_diario', (function __lambda__ () {
				...
				return self.abrir_diario (window.PhanterPWA.XWAY ('diario-de-notas', 'professor', self.id_escola, self.ano_letivo, self.id_turma, $ (this).data ('id_aluno')));
			}));
		}
		else {
			$ ('.linha_aluno_abrir_diario').off ('click.aluno_abrir_diario').on ('click.aluno_abrir_diario', (function __lambda__ () {
				...
				return self.abrir_diario (window.PhanterPWA.XWAY ('diario-de-notas', self.id_escola, self.ano_letivo, self.id_turma, $ (this).data ('id_aluno')));
			}));
		}
	});},
	...
	get onKeyPress () {return __get__ (this, function (self, event, el) {
		...
		var element = $ (el);
		if (code == 13) {
			var proximo_campo = element.parent ().data ('proximo_campo');
			$ ('#{0}'.format (proximo_campo)).find ('input').focus ().select ();
		}
	});},
	...
	get _get_diario_de_notas () {return __get__ (this, function (self) {
		...
		if (self.id_aluno !== null && self.id_aluno !== undefined) {
			if (self.eh_professor) {
				window.PhanterPWA.GET ('api', 'diario-de-notas', 'professor', self.id_escola, self.ano_letivo, self.id_turma, self.id_aluno, __kwargtrans__ ({onComplete: self.after_get}));
			}
			else {
				window.PhanterPWA.GET ('api', 'diario-de-notas', self.id_escola, self.ano_letivo, self.id_turma, self.id_aluno, __kwargtrans__ ({onComplete: self.after_get}));
			}
		}
