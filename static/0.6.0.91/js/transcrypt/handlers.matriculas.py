            ...
            if matriculas is not js_undefined:
                ...
                for x in matriculas:
                    if x.turma not in lista_de_turma:
                        ...
                        table = XTABLE(
                            "matriculas-table",
                            THEAD(TR(TH(XML("&#160;"), _colspan=7, _style="border-top: hidden; border-left: hidden; border-right: hidden;"))),
                            TFOOT(TR(TH(XML("&#160;"), _colspan=7, _style="border-bottom: hidden; border-left: hidden; border-right: hidden;"))),
                            TR(TH(x.turma, _colspan=7, _style="text-align: center; color: white; background-color: grey;")),
                            XTRH(
                                "matriculas-table-head",
                                "Nº",
                                "Aluno(a)",
                    ...
                    if x.eh_multisseriado:
                        if x.serie not in lista_de_serie:
                            ...
                            table.append(
                                TR(TH(x.serie, _colspan=7, _style="text-align: center; color: orange;"), _style="background-color: #f7f7f7;border-top: solid 1px rgba(51, 51, 51, 0.12);")
                            )
                    ...
                    table.append(
                        XTRD(
                            ...
                            **{"drag_and_drop": False, "_class": self._class_resultados(x.resultados)}
                        )
                    )
    ...
    def depois_de_confirmar_desistencia(self, data, ajax_status):
        if ajax_status == "success":
            ...
            html_xd = XTRD(
                ...
                **{"drag_and_drop": False, "_class": self._class_resultados(json.data_matricula.resultados)}
            )
