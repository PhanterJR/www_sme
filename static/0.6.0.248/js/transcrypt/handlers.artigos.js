// Transcrypt'ed from Python, 2022-02-03 06:58:10
		...
		if (ajax_status == 'success') {
			...
			if (json.data !== undefined && json.data !== null) {
				...
				for (var x of json.data) {
					...
					var result = DIV (md.render (x.texto), __kwargtrans__ ({_class: 'phanterpwa-markdownit-wrapper'}));
					html_artigos.append (DIV (H2 (x.titulo), DIV (DIV (STRONG ('Autor: '), x.autor, __kwargtrans__ ({_class: 'p-col w1p50'})), DIV (STRONG ('Postagem: '), data_postagem, __kwargtrans__ ({_class: 'p-col w1p50'})), __kwargtrans__ ({_class: 'p-row'})), XML (result), __kwargtrans__ ({_id: 'artigo-container-{0}'.format (x.id), _class: 'artigo-container'})));
				}
