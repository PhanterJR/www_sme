// Transcrypt'ed from Python, 2021-10-27 01:57:58
	...
	get __init__ () {return __get__ (this, function (self, target_element) {
		...
		if (self._has_social_logins) {
			for (var x of self._social_logins) {
				...
				self.xml_social_logins.append (DIV (DIV (icon, I18N ('Login with {0}'.format (str (social_name).capitalize ())), __kwargtrans__ (dict ({'_class': 'btn btn-social_login link', '_data-social_login': social_name}))), __kwargtrans__ ({_class: 'btn-social_login-wrapper'})));
			}
		}
...
export var ModalRegister =  __class__ ('ModalRegister', [modal.Modal], {
	...
	get __init__ () {return __get__ (this, function (self, target_element) {
		...
		if (__in__ ('SMS', window.PhanterPWA.CONFIG)) {
			if (self.user_mobile_number === true) {
				self._xml_button = DIV (DIV (DIV (I (__kwargtrans__ ({_class: 'fas fa-envelope'})), I18N ('Register with {0}'.format (str ('email').capitalize ()), __kwargtrans__ (dict ({'_pt-br': 'Registrar com o {0}'.format (str ('email').capitalize ())}))), __kwargtrans__ (dict ({'_class': 'btn btn-social_login link', '_data-social_login': 'email'}))), __kwargtrans__ ({_class: 'btn-social_login-wrapper'})), __kwargtrans__ ({_style: 'display: table;margin: auto;'}));
			}
			else {
				self._xml_button = DIV (DIV (DIV (I (__kwargtrans__ ({_class: 'fas fa-mobile-alt'})), I18N ('Register with {0}'.format (str ('mobile').capitalize ()), __kwargtrans__ (dict ({'_pt-br': 'Registrar com o {0}'.format (str ('mobile').capitalize ())}))), __kwargtrans__ (dict ({'_class': 'btn btn-social_login link', '_data-social_login': 'mobile'}))), __kwargtrans__ ({_class: 'btn-social_login-wrapper'})), __kwargtrans__ ({_style: 'display: table;margin: auto;'}));
			}
		}
		var tcontent = DIV (self._xml_button, forms.FormWidget ('register', 'first_name', __kwargtrans__ (dict ({'type': 'string', 'label': I18N ('First Name'), 'validators': ['IS_NOT_EMPTY'], '_class': 'p-col w1p100 w3p50'}))), forms.FormWidget ('register', 'last_name', __kwargtrans__ (dict ({'type': 'string', 'label': I18N ('Last Name'), 'validators': ['IS_NOT_EMPTY'], '_class': 'p-col w1p100 w3p50'}))), input_name, forms.FormWidget ('register', 'password', __kwargtrans__ (dict ({'type': 'password', 'label': I18N ('Password'), 'value': password, 'validators': ['IS_NOT_EMPTY', 'IS_EQUALS:#phanterpwa-widget-input-input-register-password_repeat'], '_class': 'p-col w1p100 w3p50'}))), forms.FormWidget ('register', 'password_repeat', __kwargtrans__ (dict ({'type': 'password', 'label': I18N ('Repeat Password'), 'validators': ['IS_NOT_EMPTY', 'IS_EQUALS:#phanterpwa-widget-input-input-register-password'], '_class': 'p-col w1p100 w3p50'}))), __kwargtrans__ ({_class: 'phanterpwa-register-form-inputs p-row'})).jquery ();
		if (self.auth_user !== null && self.auth_user !== undefined) {
			tcontent.addClass ('has_auth_user');
		}
...
export var ModalRequestPassword =  __class__ ('ModalRequestPassword', [modal.Modal], {
	...
	get __init__ () {return __get__ (this, function (self, target_element) {
		...
		if (__in__ ('SMS', window.PhanterPWA.CONFIG)) {
			if (self.user_mobile_number === true) {
				self._xml_button = DIV (DIV (DIV (I (__kwargtrans__ ({_class: 'fas fa-envelope'})), I18N ('Recover with {0}'.format (str ('email').capitalize ())), __kwargtrans__ (dict ({'_class': 'btn btn-social_login link', '_data-social_login': 'email'}))), __kwargtrans__ ({_class: 'btn-social_login-wrapper'})), __kwargtrans__ ({_style: 'display: table;margin: auto;'}));
			}
			else {
				self._xml_button = DIV (DIV (DIV (I (__kwargtrans__ ({_class: 'fas fa-mobile-alt'})), I18N ('Recover with {0}'.format (str ('mobile').capitalize ())), __kwargtrans__ (dict ({'_class': 'btn btn-social_login link', '_data-social_login': 'mobile'}))), __kwargtrans__ ({_class: 'btn-social_login-wrapper'})), __kwargtrans__ ({_style: 'display: table;margin: auto;'}));
			}
		}
		var tcontent = DIV (self._xml_button, widget_email, __kwargtrans__ ({_class: 'phanterpwa-request_password-form-inputs p-row'})).jquery ();
		if (self.auth_user !== null && self.auth_user !== undefined) {
			tcontent.addClass ('has_auth_user');
		}
	...
	get submit () {return __get__ (this, function (self) {
		...
		self.clear_errors ();
		if (self.user_mobile_number) {
			window.PhanterPWA.request_password ($ ('#phanterpwa-widget-input-input-request_password-csrf_token').val (), $ ('#phanterpwa-widget-input-input-request_password-email').val (), __kwargtrans__ ({callback: self.after_submit, user_mobile_number: true}));
		}
		else {
