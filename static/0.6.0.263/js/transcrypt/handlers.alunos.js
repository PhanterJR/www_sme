// Transcrypt'ed from Python, 2022-02-17 23:36:33
...
export var Alunos =  __class__ ('Alunos', [helpers.XmlConstructor], {
	...
	get abrir_modal_visualizar () {return __get__ (this, function (self, data, ajax_status) {
		...
		if (ajax_status == 'success') {
			...
			var card = DIV (matriculas.Visualizar (id_matricula, id_escola, ano_letivo, prof_pai, prof_mae, id_aluno, nome_do_aluno, sexo, data_de_nascimento, nome_do_pai, nome_da_mae, naturalidade, endereco, serie, nome_do_responsavel, data_mat, novato, admitido, unidade_admitido, ano_anterior, serie_ant, resultado_anterior, numero_aluno, turma), DIV (DIV (widgets.FloatMenu ('menu_matricula_{0}'.format (id_matricula), I (__kwargtrans__ ({_class: 'fas fa-ellipsis-v'})), widgets.FloatButton (I (DIV (DIV (SPAN (I (__kwargtrans__ ({_class: 'fas fa-user-graduate'})), __kwargtrans__ ({_class: 'icombine-container-first'})), SPAN (I (__kwargtrans__ ({_class: 'fas fa-plus'})), __kwargtrans__ ({_class: 'icombine-container-last'})), __kwargtrans__ ({_class: 'icombine-container'})), __kwargtrans__ ({_class: 'phanterpwa-snippet-icombine'}))), __kwargtrans__ ({_class: 'botao_matricular', _title: 'Nova matrícula', _href: window.PhanterPWA.XWAY ('matricular', id_escola, ano_letivo, id_aluno)})), widgets.FloatButton (I (__kwargtrans__ ({_class: 'fas fa-edit'})), __kwargtrans__ ({_class: 'botao_editar_matricula', _title: 'Editar Matrícula', _href: window.PhanterPWA.XWAY ('matricula', id_aluno, id_escola, ano_letivo, 'aluno-conferido')})), widgets.FloatButton (I (__kwargtrans__ ({_class: 'fas fa-comments'})), __kwargtrans__ ({_class: 'botao_questionario', _title: 'Questionário Social', _href: window.PhanterPWA.XWAY ('imprimir', 'questionario-social', id_escola, ano_letivo, id_aluno, __kwargtrans__ ({_id_matricula: id_matricula}))})), widgets.FloatButton (I (__kwargtrans__ ({_class: 'fas fa-file-pdf'})), __kwargtrans__ ({_class: 'botao_gerar_pdf', _title: 'Gerar PDF', _href: '{0}/api/pdfs/matricula/{1}?nocache={2}'.format (window.PhanterPWA.ApiServer.remote_address, id_matricula, now)})), widgets.FloatButton (I (__kwargtrans__ ({_class: 'fas fa-print'})), __kwargtrans__ ({_title: 'Imprimir documento', _class: 'botao_imprimir_diario_de_notas', _onclick: 'print();'})), __kwargtrans__ ({_style: 'position: absolute;'})), __kwargtrans__ ({_style: 'position: relative;', _class: 'p-col w1p100'})), __kwargtrans__ ({_class: 'p-row'})));
			self.modal_visualizar = modal.Modal ('#modal_visualizar_aluno', __kwargtrans__ (dict ({'title': CONCATENATE (DIV ('Visualizar Matrícula ', STRONG (ano_letivo)), DIV (STRONG (json.data.nome_do_aluno))), 'content': card})));
			self.modal_visualizar.open ();
		}
		else {
			window.PhanterPWA.flash ('Não há alunos_visualizar matriculados para a série da turma');
...
export var Aluno =  __class__ ('Aluno', [helpers.XmlConstructor], {
	...
	get binds () {return __get__ (this, function (self) {
		...
		$ ('#phanterpwa-widget-input-input-alunos-nome_do_pai').off ('change.update_responsaveis').on ('change.update_responsaveis', (function __lambda__ () {
			if (arguments.length) {
				var __ilastarg0__ = arguments.length - 1;
				if (arguments [__ilastarg0__] && arguments [__ilastarg0__].hasOwnProperty ("__kwargtrans__")) {
					var __allkwargs0__ = arguments [__ilastarg0__--];
			...
			return self.update_responsaveis ();
		}));
		$ ('#phanterpwa-widget-input-input-alunos-nome_da_mae').off ('change.update_responsaveis').on ('change.update_responsaveis', (function __lambda__ () {
			if (arguments.length) {
				var __ilastarg0__ = arguments.length - 1;
				if (arguments [__ilastarg0__] && arguments [__ilastarg0__].hasOwnProperty ("__kwargtrans__")) {
					var __allkwargs0__ = arguments [__ilastarg0__--];
