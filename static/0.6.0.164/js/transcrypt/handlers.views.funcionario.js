// Transcrypt'ed from Python, 2021-08-16 10:08:40
...
export var Funcionario =  __class__ ('Funcionario', [helpers.XmlConstructor], {
	...
	get __init__ () {return __get__ (this, function (self, index_instance, id_funcionario, action) {
		...
		self.action = action;
		var html = CONCATENATE (DIV (DIV (DIV (DIV ('FUNCIONÁRIOS', __kwargtrans__ ({_class: 'phanterpwa-breadcrumb'})), DIV ('FUNCIONÁRIO', __kwargtrans__ ({_class: 'phanterpwa-breadcrumb'})), __kwargtrans__ ({_class: 'phanterpwa-breadcrumb-wrapper'})), __kwargtrans__ ({_class: 'p-container'})), __kwargtrans__ ({_class: 'title_page_container card'})), DIV (DIV (DIV (DIV (preloaders.android, __kwargtrans__ ({_style: 'width: 300px; height: 300px; overflow: hidden; margin: auto;'})), __kwargtrans__ ({_style: 'text-align:center; padding: 50px 0;'})), __kwargtrans__ ({_id: 'content-funcionarios', _class: 'p-row card e-padding_20'})), __kwargtrans__ ({_class: 'phanterpwa-container p-container'})));
