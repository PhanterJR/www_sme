// Transcrypt'ed from Python, 2021-11-21 18:45:06
		...
		if (self.current_hash !== json.hash) {
			...
			if (matriculas_sem_turma !== undefined) {
				...
				for (var x of matriculas_sem_turma) {
					...
					var ordem = 0;
					if (cont_sem_turma == 1) {
						var table = XTABLE ('matriculas-table-sem_turma', THEAD (TR (TH (XML ('&#160;'), __kwargtrans__ ({_colspan: 8, _style: 'border-top: hidden; border-left: hidden; border-right: hidden;'})))), TFOOT (TR (TH (XML ('&#160;'), __kwargtrans__ ({_colspan: 8, _style: 'border-bottom: hidden; border-left: hidden; border-right: hidden;'})))), TR (TH ('ALUNOS MATRICULADOS SEM TURMA', __kwargtrans__ ({_colspan: 8, _style: 'text-align: center; color: white; background-color: red;'}))), XTRH ('matriculas-table-head-sem_turma', 'Aluno(a)', 'Data Nascimento', 'Nome do Pai', 'Nome da Mãe', 'Endereço', TH (DIV (I (__kwargtrans__ ({_class: 'fas fa-plus'})), __kwargtrans__ (dict ({'_phanterpwa-way': 'matricula/novo', '_class': 'icon_button wave_on_click'}))), __kwargtrans__ ({_class: 'nao_imprimir'}))));
