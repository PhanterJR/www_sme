// Transcrypt'ed from Python, 2022-02-21 22:38:20
...
export var IdentificarAluno =  __class__ ('IdentificarAluno', [object], {
	...
	get etapa1 () {return __get__ (this, function (self) {
		...
		var titulo = CONCATENATE (TABLE (TR (TD ('SISTEMA DE INDENTIFICAÇÃO DO ALUNO')), __kwargtrans__ ({_id: 'nome_escola_chat'})), DIV (I (__kwargtrans__ ({_class: 'fas fa-expand'})), __kwargtrans__ ({_class: 'botao_expand'})));
	...
	get _get_codigo_de_acesso () {return __get__ (this, function (self, change) {
		...
		if (self.sabe_codigo_de_acesso === null || change || self.fazer_alteracoes) {
			if (change || self.fazer_alteracoes) {
				var resp = CONCATENATE (P ('Quero tentar adicionar um ', STRONG ('CÓDIGO DE ACESSO'), '.', __kwargtrans__ ({_class: 'rclient'})), P ('Tudo bem. Você está pronto para adicionar o ', STRONG ('CÓDIGO DE ACESSO'), '?.', __kwargtrans__ ({_class: 'remsc'})));
			}
			else {
				var resp = CONCATENATE (P ('Se vc souber o ', STRONG ('CÓDIGO DE ACESSO'), ' a identificação ocorre rapidamente. Ao fazer a matrícula na escola é fornecido um COMPROVANTE DE MATRÍCULA', ' , se você tem este documento, o ', STRONG ('CÓDIGO DE ACESSO'), ' está logao abaixo do ', STRONG ('QRCODE'), '.', __kwargtrans__ ({_class: 'remsc'})));
			}
			...
			var choice = DIV (BUTTON ('EU TENHO O CÓDIGO DE ACESSO', __kwargtrans__ ({_id: 'sim_codigo_de_acesso', _class: 'btn wave-on-click'})), BUTTON ('EU NÃO TENHO O CÓDIGO DE ACESSO', __kwargtrans__ ({_id: 'nao_codigo_de_acesso', _class: 'btn wave-on-click'})), __kwargtrans__ ({_class: 'buttons_choice'}));
	...
	get _sabe_codigo_de_acesso () {return __get__ (this, function (self, sabe, change) {
		...
		if (self.sabe_codigo_de_acesso === true || change || self.fazer_alteracoes) {
			var resp = CONCATENATE (P ('Eu tenho o ', STRONG ('CÓDIGO DE ACESSO'), '.', __kwargtrans__ ({_class: 'rclient'})), P ('Ok, adicione o seu ', STRONG ('CÓDIGO DE ACESSO'), '.', __kwargtrans__ ({_class: 'remsc'})));
			var arbritary_id = window.PhanterPWA.get_id ();
			var mensagem = DIV (resp, __kwargtrans__ ({_id: arbritary_id, _style: 'display: none;'}));
			mensagem.append_to ('#row_content');
			$ ('#{0}'.format (arbritary_id)).slideDown (500);
			$ ('#row_content').animate (dict ({'scrollTop': 2000000}), 'slow');
			self.wd_cod_acesso = widgets.Input ('codigo_de_acesso', __kwargtrans__ ({icon: I (__kwargtrans__ ({_class: 'fab fa-telegram-plane'})), placeholder: 'Código de Acesso', wear: 'shadows', checker: false, value: (self.fazer_alteracoes && self.codigo_de_acesso !== null ? self.codigo_de_acesso : '')}));
			...
			var mensagem = DIV (P (SPAN (botao_edit), 'Não tenho o ', STRONG ('CÓDIGO DE ACESSO'), '.', __kwargtrans__ ({_class: 'rclient'})), P ('Ok, vamos pular esta parte e tentar outra alternativa.', __kwargtrans__ ({_class: 'remsc'})), __kwargtrans__ ({_id: arbritary_id, _style: 'display: none;'}));
			mensagem.append_to ('#row_content');
			$ ('#{0}'.format (arbritary_id)).slideDown (500);
			$ ('#row_content').animate (dict ({'scrollTop': 2000000}), 'slow');
	...
	get _validate_codigo_de_acesso () {return __get__ (this, function (self) {
		...
		if (value !== null && value !== undefined && value != '' && value.length > 0) {
			...
			var arbritary_id_menu = window.PhanterPWA.get_id ();
			var arbritary_id_choice = window.PhanterPWA.get_id ();
			var botao_edit = widgets.MenuBox (arbritary_id_menu, I (__kwargtrans__ ({_class: 'fas fa-edit'})), DIV ('Quero mudar o código de acesso', __kwargtrans__ ({_id: arbritary_id_choice})), __kwargtrans__ (dict ({'_class': 'button_editar', 'z_index': 2001, 'onOpen': (function __lambda__ () {
				if (arguments.length) {
					var __ilastarg0__ = arguments.length - 1;
					if (arguments [__ilastarg0__] && arguments [__ilastarg0__].hasOwnProperty ("__kwargtrans__")) {
						var __allkwargs0__ = arguments [__ilastarg0__--];
			...
			var mensagem = DIV (P (botao_edit, self.codigo_de_acesso, __kwargtrans__ ({_class: 'rclient'})), p ('Checando o código digitado... Aguarde...'), __kwargtrans__ ({_id: arbritary_id, _style: 'display: none;'}));
			mensagem.append_to ('#row_content');
			$ ('#{0}'.format (arbritary_id)).slideDown (500);
			$ ('#row_content').animate (dict ({'scrollTop': 2000000}), 'slow');
	...
	get _get_nome_completo () {return __get__ (this, function (self, change, codigo_invalido) {
		...
		if (self.nome_completo === null || change || self.fazer_alteracoes) {
			if (codigo_invalido === true) {
				var xml_ini = P ('O código adicionado não foi reconhecido, vamos tentar outra alternativa. ', 'Digite agora seu nome completo. Lembrando que o nome deve ser digitado', ' exatamente como está na certidão de nascimento.', __kwargtrans__ ({_class: 'remsc'}));
			}
			else {
				var xml_ini = P ('Me informe o seu nome completo. Lembrando que o nome deve ser digitado', 'exatamente como está na certidão de nascimento.', __kwargtrans__ ({_class: 'remsc'}));
			}
			if (change || self.fazer_alteracoes) {
				var resp = CONCATENATE (P ('Quero mudar o nome completo.', __kwargtrans__ ({_class: 'rclient'})), P ('Tudo bem, digite agora seu nome completo. Lembrando que o nome deve ser digitado', ' exatamente como está na certidão de nascimento.', __kwargtrans__ ({_class: 'remsc'})));
			}
			else {
				var resp = xml_ini;
			}
	...
	get after_submit () {return __get__ (this, function (self, data, ajax_status) {
		...
		if (ajax_status == 'success') {
			...
			var codigo_de_acesso = SPAN ('Está codigo_de_acessod', artigo, ' no(a) ', STRONG (data.servidor.turma));
			var arbritary_id = window.PhanterPWA.get_id ();
	...
	get after_submit2 () {return __get__ (this, function (self, data, ajax_status) {
		...
		if (ajax_status == 'success') {
			...
			if (self.codigo_de_acesso == '' || self.codigo_de_acesso === null) {
				var cod_msg = 'Não sei o código de acesso';
			}
			var arbritary_id = window.PhanterPWA.get_id ();
			var mensagem = DIV (P ('Não consegui te identificar com os dados que você me deu...', __kwargtrans__ ({_class: 'remsc'})), P ('Nome completo: ', STRONG (self.nome_completo), BR (), 'Código de Acesso: ', STRONG (cod_msg), BR (), 'Data de Nascimento: ', STRONG (self.data_de_nascimento), BR (), 'Nome da mãe: ', STRONG (self.nome_da_mae), BR (), 'Celular: ', STRONG (self.numero_celularfunc), __kwargtrans__ ({_class: 'remsc'})), P ('Pode ser que você tenha digitado algo errado!', ' Faça as alterações necessárias em ', STRONG ('CONSERTAR ALGO ERRADO'), ' que eu tento te identificar novamente.', ' Se você tem certeza que os dados inseridos estão corretos, o jeito é falar com alguém da administração do SME', ' ou você pode (opção mais recomendada) deixar uma mensagem em ', STRONG ('MANDAR MENSAGEM'), __kwargtrans__ ({_class: 'remsc'})), __kwargtrans__ ({_id: arbritary_id, _style: 'display: none;'}));
			mensagem.append_to ('#row_content');
			$ ('#{0}'.format (arbritary_id)).slideDown (500);
			$ ('#row_content').animate (dict ({'scrollTop': 2000000}), 'slow');
			var choice = DIV (widgets.MenuBox ('alterar_dados', BUTTON ('CONSERTAR ALGO ERRADO', __kwargtrans__ ({_id: 'alterar_dados', _class: 'btn wave-on-click'})), DIV ('Quero tentar acessar usando o CÓDIGO DE ACESSO', __kwargtrans__ ({_id: 'algo_errado_codigo_de_acesso'})), DIV ('Quero mudar o Nome Completo', __kwargtrans__ ({_id: 'algo_errado_nome_completo'})), DIV ('Quero mudar o CPF', __kwargtrans__ ({_id: 'algo_errado_cpf'})), DIV ('Quero mudar o Data de Nascimento', __kwargtrans__ ({_id: 'algo_errado_data_de_nascimento'})), DIV ('Quero mudar o Nome da Mãe', __kwargtrans__ ({_id: 'algo_errado_nome_da_mae'})), DIV ('Quero mudar o Número Celular', __kwargtrans__ ({_id: 'algo_errado_numero_celular'})), __kwargtrans__ (dict ({'_class': 'button_editar', '_style': 'width: auto; display: inline-block;', 'z_index': 2001, 'onOpen': (function __lambda__ () {
				if (arguments.length) {
					var __ilastarg0__ = arguments.length - 1;
					if (arguments [__ilastarg0__] && arguments [__ilastarg0__].hasOwnProperty ("__kwargtrans__")) {
						var __allkwargs0__ = arguments [__ilastarg0__--];
