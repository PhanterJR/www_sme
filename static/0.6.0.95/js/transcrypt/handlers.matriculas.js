// Transcrypt'ed from Python, 2021-10-22 20:11:12
...
export var Matriculas =  __class__ ('Matriculas', [object], {
	...
	get process_data () {return __get__ (this, function (self, json) {
		...
		if (self.current_hash !== json.hash) {
			...
			var table = XTABLE ('matriculas-table', XTRH ('matriculas-table-head', 'ID', 'Aluno(a)', 'Data Nascimento', 'Nome do Pai', 'Nome da Mãe', 'Endereço', DIV (I (__kwargtrans__ ({_class: 'fas fa-plus'})), __kwargtrans__ (dict ({'_phanterpwa-way': 'matricula/novo', '_class': 'icon_button wave_on_click'})))));
			if (matriculas !== undefined) {
				...
				for (var x of matriculas) {
					if (!__in__ (x.turma, lista_de_turma)) {
						...
						var lista_de_serie = [];
						var ordem = 0;
						var table = XTABLE ('matriculas-table', THEAD (TR (TH (XML ('&#160;'), __kwargtrans__ ({_colspan: 7, _style: 'border-top: hidden; border-left: hidden; border-right: hidden;'})))), TFOOT (TR (TH (XML ('&#160;'), __kwargtrans__ ({_colspan: 7, _style: 'border-bottom: hidden; border-left: hidden; border-right: hidden;'})))), TR (TH (x.turma, __kwargtrans__ ({_colspan: 7, _style: 'text-align: center; color: white; background-color: grey;'}))), XTRH ('matriculas-table-head', 'Nº', 'Aluno(a)', 'Data Nascimento', 'Nome do Pai', 'Nome da Mãe', 'Endereço', TH (DIV (I (__kwargtrans__ ({_class: 'fas fa-plus'})), __kwargtrans__ (dict ({'_phanterpwa-way': 'matricula/novo', '_class': 'icon_button wave_on_click'}))), __kwargtrans__ ({_class: 'nao_imprimir'}))));
						html_tabelas.append (table);
					}
					if (x.eh_multisseriado) {
						if (!__in__ (x.serie, lista_de_serie)) {
							lista_de_serie.append (x.serie);
							table.append (TR (TH (x.serie, __kwargtrans__ ({_colspan: 7, _style: 'text-align: center; color: orange;'})), __kwargtrans__ ({_style: 'background-color: #f7f7f7;border-top: solid 1px rgba(51, 51, 51, 0.12);'})));
						}
					}
					...
					table.append (XTRD ('matriculas-table-data-{0}'.format (x.id), ordem, x.aluno, data_de_nascimento_formated, x.nome_do_pai, x.nome_da_mae, x.endereco, TD (widgets.MenuBox ('drop_{0}'.format (x.id), I (__kwargtrans__ ({_class: 'fas fa-ellipsis-v'})), widgets.MenuOption ('Dados do Aluno', __kwargtrans__ (dict ({'_class': 'botao_visualizar_aluno wave_on_click', '_data-id_aluno': x.id_aluno}))), widgets.MenuOption ('Visualizar Matrícula', __kwargtrans__ (dict ({'_class': 'botao_visualizar_matricula wave_on_click', '_data-id_matricula': x.id}))), widgets.MenuOption ('Desistência', __kwargtrans__ (dict ({'_class': 'botao_aluno_desistente wave_on_click', '_data-id_matricula': x.id}))), widgets.MenuOption ('Transferência', __kwargtrans__ (dict ({'_class': 'botao_transferir_aluno wave_on_click', '_data-id_matricula': x.id}))), __kwargtrans__ ({onOpen: (function __lambda__ () {
						if (arguments.length) {
							var __ilastarg0__ = arguments.length - 1;
							if (arguments [__ilastarg0__] && arguments [__ilastarg0__].hasOwnProperty ("__kwargtrans__")) {
								var __allkwargs0__ = arguments [__ilastarg0__--];
					...
					})})), __kwargtrans__ ({_class: 'nao_imprimir'})), __kwargtrans__ (dict ({'drag_and_drop': false, '_class': self._class_resultados (x.resultados)}))));
				}
			}
	...
	get abrir_modal_desistencia () {return __get__ (this, function (self, el) {
		...
		var content = CONCATENATE (texto1, texto2, DIV (widgets.Select ('unidade_des', __kwargtrans__ ({label: 'Quando desistiu?', value: '', can_empty: true, data_set: data_set})), __kwargtrans__ ({_id: 'phanterpwa-input-search_field-matriculas', _class: 'p-col w1p100'})));
	...
	get depois_de_confirmar_desistencia () {return __get__ (this, function (self, data, ajax_status) {
		...
		if (ajax_status == 'success') {
			...
			var html_xd = XTRD ('matriculas-table-data-{0}'.format (json.data_matricula.id), numero_aluno, json.data_matricula.aluno, data_de_nascimento_formated, json.data_matricula.nome_do_pai, json.data_matricula.nome_da_mae, json.data_matricula.endereco, TD (widgets.MenuBox ('drop_{0}'.format (json.data_matricula.id), I (__kwargtrans__ ({_class: 'fas fa-ellipsis-v'})), widgets.MenuOption ('Dados do Aluno', __kwargtrans__ (dict ({'_class': 'botao_visualizar_aluno wave_on_click', '_data-id_aluno': json.data_matricula.id_aluno}))), widgets.MenuOption ('Visualizar Matrícula', __kwargtrans__ (dict ({'_class': 'botao_visualizar_matricula wave_on_click', '_data-id_matricula': json.data_matricula.id}))), widgets.MenuOption ('Desistência', __kwargtrans__ (dict ({'_class': 'botao_aluno_desistente wave_on_click', '_data-id_matricula': json.data_matricula.id}))), widgets.MenuOption ('Transferência', __kwargtrans__ (dict ({'_class': 'botao_transferir_aluno wave_on_click', '_data-id_matricula': json.data_matricula.id}))), __kwargtrans__ ({onOpen: (function __lambda__ () {
				...
				return self.binds_menu_aluno ();
			})})), __kwargtrans__ ({_class: 'nao_imprimir'})), __kwargtrans__ (dict ({'drag_and_drop': false, '_class': self._class_resultados (json.data_matricula.resultados)})));
	...
	get abrir_modal_transferencia () {return __get__ (this, function (self, el) {
		...
		data_set.append ('Fim de Curso');
		var content = CONCATENATE (texto1, texto2, DIV (widgets.Select ('unidade_trams', __kwargtrans__ ({label: 'Quando foi a Transferência?', value: '', can_empty: true, data_set: data_set})), __kwargtrans__ ({_id: 'phanterpwa-input-search_field-matriculas', _class: 'p-col w1p100'})));
