    ...
    def initialize(self):
        ...
        if arg0 == "calendario-base":
            ...
            if arg0 is None or arg0 is js_undefined:
                ...
                if arg2 == "editar" and window.PhanterPWA.auth_user_has_role(["administrator", "root", "Administrador Master SME", "Administrador Master Escola", "Coordenação SME"]):
    ...
    def xml_modal_unidade(self):
        ...
        for x in range(self.quant_unidades):
            ...
            if str(key) in dict_periodo_unidades.keys():
                ...
                final = TD(widgets.Inert("df_unidade_{0}".format(x + 1), **{
                    "value": self.iso_br(dict_periodo_unidades[x + 1][2])
                }))
                if x + 1 == self.quant_unidades:
                    final = forms.FormWidget(
                        "calendario_unidade",
                        "data_final_unidade_{0}".format(x + 1),
                        **{
                            "value": self.data_final_ultima_unidade,
                            "label": "Data Final",
                            "type": "date",
                            "validators": ["IS_EMPTY_OR", "IS_DATE:dd/MM/yyyy"],
                            "validator_format": "%d/%m/%Y",
