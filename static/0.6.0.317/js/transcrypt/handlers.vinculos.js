// Transcrypt'ed from Python, 2022-04-19 19:23:39
		...
		if (ajax_status == 'success') {
			...
			for (var x of dados) {
				...
				if (!__in__ (x [0], escolas)) {
					...
					var card = DIV (LABEL (DIV (IMG (__kwargtrans__ ({_src: endereco_imagem_escola, _style: 'width:25px; height:25px; border-radius: 100%;position: absolute;top: 6px;'})), __kwargtrans__ ({_style: 'display: inline-block;margin-right: 10px;height: 25px;width: 25px;position: relative;'})), x [1], __kwargtrans__ ({_for: 'phanterpwa-card-panel-control-{0}'.format (x [0])})), DIV (DIV (DIV (DIV ((bool_equipe_diretiva ? equipe_diretiva : ''), (bool_equipe_diretiva && bool_corpo_docente ? BR () : ''), (bool_corpo_docente ? corpo_docente : ''), (bool_equipe_diretiva && bool_equipe_de_apoio ? BR () : ''), (bool_equipe_de_apoio ? equipe_de_apoio : ''), __kwargtrans__ ({_class: 'p-row'})), __kwargtrans__ ({_class: 'phanterpwa-card-panel-control-content'})), DIV (A (I (__kwargtrans__ ({_class: 'fas fa-print'})), __kwargtrans__ (dict ({'_class': 'botao_imprimir_lista_alunos_turma icon_button', '_title': 'Imprimir lista de alunos', '_href': window.PhanterPWA.XWAY ('imprimir', 'vinculos', self.arg1, self.arg2)}))), __kwargtrans__ ({_class: 'phanterpwa-card-panel-control-buttons'})), __kwargtrans__ ({_class: 'phanterpwa-card-panel-control-wrapper has_buttons'})), __kwargtrans__ ({_class: 'phanterpwa-card-panel-control-container'})), __kwargtrans__ ({_id: 'phanterpwa-card-panel-control-{0}'.format (x.id_escola), _class: 'phanterpwa-card-panel-control'}));
					xmls.append (card);
				}
			}
