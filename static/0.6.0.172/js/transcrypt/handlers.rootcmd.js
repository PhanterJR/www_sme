// Transcrypt'ed from Python, 2021-11-19 21:18:26
...
export var Index =  __class__ ('Index', [gatehandler.Handler], {
	...
	get initialize () {return __get__ (this, function (self) {
		...
		var html = CONCATENATE (DIV (DIV (DIV (DIV ('root_commands', __kwargtrans__ ({_class: 'phanterpwa-breadcrumb'})), __kwargtrans__ ({_class: 'phanterpwa-breadcrumb-wrapper'})), __kwargtrans__ ({_class: 'p-container'})), __kwargtrans__ ({_class: 'title_page_container card'})), DIV (DIV (DIV (DIV ('Ativar Contas', __kwargtrans__ ({_class: 'btn botao_ativar_contas'}))), __kwargtrans__ ({_id: 'content-root_commands', _class: 'p-row card e-padding_20'})), __kwargtrans__ ({_class: 'phanterpwa-container p-container'})));
		html.html_to ('#main-container');
		$ ('.botao_ativar_contas').on ('').off ('');
	});}
});

//# sourceMappingURL=handlers.rootcmd.map