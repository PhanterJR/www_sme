...
class Matricula(helpers.XmlConstructor):
    ...
    def process_data(self, json):
        ...
        if turma is not None and turma is not js_undefined:
            texto = CONCATENATE(
                P("Matricula de: ", STRONG(nome_aluno, _style="color: orange; text-transform: uppercase;")),
                P(P1, "JÁ POSSUI UMA MATRÍCULA NO ANO LETIVO DE ", STRONG(ano_letivo), ", PORÉM NÃO É POSSÍVEL EDITAR A SÉRIE, POIS ", 
                    P1, " SE ENCONTRA NA TURMA ", STRONG(turma, _style="text-transform: uppercase;"), ". PARA PODER EDITAR A SÉRIE É NECESSÁRIO ", RETIRALO," DA TURMA."),
