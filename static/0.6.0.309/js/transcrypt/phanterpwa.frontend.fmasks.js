// Transcrypt'ed from Python, 2022-04-05 08:17:54
...
export var Mask =  __class__ ('Mask', [object], {
	...
	get onKeyUp () {return __get__ (this, function (self, event, el) {
		var element = $ (el);
		...
		if (__in__ (code, noprintkeys)) {
			if (code == 8) {
				...
				if (self.stringFilter (current_value) !== '') {
					var pure_value = self.stringFilter (current_value);
					var pure_value = pure_value.__getslice__ (0, -(1), 1);
					var new_value = self.mask_function (pure_value) [0];
					if (pure_value !== '') {
						element.val (new_value);
					}
	...
	get onFocusOut () {return __get__ (this, function (self, event, el) {
		var element = $ (el);
		var new_value = element.val ();
		var pure_value = self.stringFilter (new_value);
		var new_value = self.mask_function (pure_value) [0];
	...
	get start () {return __get__ (this, function (self) {
		...
		element.off ('focusout.phanterpwaMask').on ('focusout.phanterpwaMask', (function __lambda__ (event) {
			return self.onFocusOut (event, this);
		}));
		element.off ('keyup.phanterpwaMask2,').on ('keyup.phanterpwaMask2,', (function __lambda__ (event) {
			return self.onKeyUp (event, this);
		}));
...
export var baseCustom = function (value, custom_mask, cursorPosition) {
	...
	if (new_value.indexOf ('_') > 0) {
		var cursorPosition = new_value.indexOf ('_');
	}
	else {
