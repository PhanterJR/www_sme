...
class Series(helpers.XmlConstructor):
    ...
    def escolha_de_series_xml(self, json):
        ...
        titulo =  H2("ESCOLHA AS SÉRIES/ANOS QUE SÃO OFERTADAS PELA SUA UNIDADE ESCOLAR",
        ...
        botao_extra = ""
        if json.data.tem_series_anteriores is True and self.serie_anterior is None:
            botao_extra = P(
                STRONG("Atenção!"),
                " Você pode adicionar as séries do ano anterior clicando ",
                SPAN("AQUI", _class="btn btn-autoresize btn-inline link", **{"_phanterpwa-way": "series/{0}/{1}/series-do-ano-anterior".format(self.id_escola, self.ano_letivo)}),
                "."
            )
                    ...
                    DIV(
                        DIV(
                            DIV(
                                ...
                                DIV(
                                    titulo,
                                    P("As seleções em laranja (se houver) não podem ser modificadas, pois possivelmente",
    ...
    def _get_series(self):
        parametros = {}
        if self.serie_anterior is not None:
