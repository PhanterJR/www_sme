        ...
        if window.PhanterPWA.Request.get_param("retornar") is None:
            BackButton1 = left_bar.LeftBarButton(
                ...
                **{
                    ...
                    "position": "top",
                    "show_if_way_match": r"^registro-de-aulas\/[0-9]{1,}\/[0-9]{1,}.*$"
                }
            )
...
class RegistroDeAulas():
    ...
    def texto_calc_dias(self, carga_horaria, total_dado, serie=None):
        ch = ""
        if str(carga_horaria).isdigit() and str(total_dado).isdigit():
            if carga_horaria > total_dado:
                falta = carga_horaria - total_dado
                pluf = ["m", "s"]
                pluchd = "s"
                if falta == 1:
                    pluf = ["", ""]
                ...
                if serie is None:
                    ...
                    ch = " {0}: {1} aula{2} de {3}. Falta{4} {5} aula{6};".format(
                        serie,
                        total_dado,
                        pluchd,
                        carga_horaria,
            ...
            elif carga_horaria == total_dado:
                if serie is None:
                    ch = " Todas as {0} aulas foram dadas.".format(carga_horaria)
                else:
                    ch = " {0}: Todas as {1} aulas foram dadas;".format(serie, carga_horaria)
            elif carga_horaria < total_dado:
                extra = total_dado - carga_horaria
                if serie is None:
                    ...
                    ch = " {0}: Você deu {1} aulas extras. {2} de {3};".format(
                        serie,
                        extra,
                        total_dado,
                        carga_horaria
    ...
    def calc_dias_dados(self):
        ...
        if self.eh_multisseriado:
            for y in self.ordem_series:
                nome_serie = self.dict_series[y]
                carga_horaria = self.cargas_horarias[y]
                carga_horaria_dada = self.conta_aulas_dadas[y]
    ...
    def processar_registro_de_aulas(self, registro_de_aulas):
        ...
        html = DIV(
            H2("Registro de aulas da turma ", SPAN(self.turma, _style="color: #9d0c0c;")),
            _class="registro_de_aulas_container"
        )
        if self.eh_multisseriado:

            tabela = TABLE(
                TR(TH("DIA"), TH("SÉRIE"), TH("QUANT."), TH("CONTEÚDO TRABALHADO E ESTRATÉGIAS APLICADAS")),
                _class='tabela_registro_de_aulas'
            )
            if self.disciplina is not None and self.disciplina is not js_undefined:
                html = DIV(
                    H2(
                        "Registro de aulas da disciplina de ",
                        SPAN(self.disciplina, _style="color: #9d0c0c;"),
                        " da turma ",
                        SPAN(self.turma, _style="color: #9d0c0c;")
                    ),
                    _class="registro_de_aulas_container"
                )
        elif self.eh_educacao_infantil:
            tabela = TABLE(
            ...
            tabela = TABLE(
                TR(TH("DIA"), TH("QUANT."), TH("CONTEÚDO TRABALHADO E ESTRATÉGIAS APLICADAS")),
                _class='tabela_registro_de_aulas'
            )
            if self.disciplina is not None and self.disciplina is not js_undefined:
                html = DIV(
        ...
        painel = DIV(
            LABEL(self.rotulo_mes_ano, SPAN(self.calc_dias_dados(), _id="total_aulas", _style="color: green")),
            DIV(
                DIV(
                    DIV(
                        DIV(
    ...
    def depois_de_enviar_registro(self, data, ajax_status, elemento):
        if ajax_status == "success":
            ...
            self.ordem_series = json.ordem_series
            serie_atual = json.serie_atual
            nome_serie = self.dict_series[serie_atual]
