...
class ProcurarAluno(helpers.XmlConstructor):
    ...
    def binds(self):
        ...
        if ano_letivo is not None and ano_letivo is not js_undefined and str(ano_letivo).isdigit():
    ...
    def _pre_clicagem(self, el, ano_letivo):
        aluno = window.PhanterPWA.get_widget("search_matriculas").value()
        params = ""
        if aluno != "":
            __pragma__('jsiter')
            jsdict = {"aluno": aluno}
        ...
        window.PhanterPWA.open_way("matricula/{0}{1}{2}".format(
            jQuery(el).parent().data("id_matricula"),
            ano_letivo,
            params
        ))
