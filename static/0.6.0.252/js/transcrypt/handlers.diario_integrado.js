...
export var DiarioRegistroDeAulas =  __class__ ('DiarioRegistroDeAulas', [object], {
		...
		if (ajax_status == 'success') {
			...
			for (var x of self.lista_dias_letivos) {
				...
				for (var y of x [1]) {
					...
					tabela.append (TBODY (TR (TD (DIV (preloaders.run_points))), __kwargtrans__ ({_id: '{0}'.format (str (y).py_replace ('-', '_'))})));
				}
			...
			if (tem_dia_letivo) {
				...
				self._get_registro_de_aulas ();
				content.append (DIV (__kwargtrans__ ({_id: 'resumo_carga_horaria'})));
	...
	get after_get () {return __get__ (this, function (self, data, ajax_status) {
		...
		if (ajax_status == 'success') {
			...
			var mes = __left0__ [1];
			var ano = __left0__ [2];
			var id_target = '#{0}_{1}_{2}'.format (ano, mes, dia);
			...
			if (json.conteudo_educacao_infantil !== null && json.conteudo_ensino_fundamental !== null) {
				...
				linha.append (TR (TH (json.data_aula, __kwargtrans__ ({_rowspan: '2'})), TD (result1)));
